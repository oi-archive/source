...
某国有编号为 $1$ 到 $n$ 的 $n$ 座城市，还有编号为 $1$ 到 $m$ 的 $m$ 条铁路线。$i$ 号铁路线被沿途 $(s_i+1)$ 座城市分为 $s_i$ 段。$i$ 号铁路线从起点到终点依次经过 $v[i,1],$ $v[i,2],$ $\dots,$ $v[i,s]$ 号城市，城市 $v[i,j]$ 通过这条线路到城市 $v[i,j+1]$ 花费的时间为 $t[i,j]$。**注意，本题中铁路线均为单向。**  
...
以下 $m$ 行，每行描述一条铁路线：  
开头一个整数 $s_i$，之后 $(2s_i+1)$ 个整数：$v[i,1],$ $t[i,1],$ $v[i,2],$ $t[i,2],$ $v[i,3]$ $\dots$ $t[i,s_i],$ $v[i,s_i+1]$。  
...
对于 $100\%$ 的数据，$2 ⩽ n ⩽ 10^6,$ $1 ⩽ m ⩽ 10^6,$ $1 ⩽ v[i,j] ⩽ n,$ $1 ⩽ t[i,j] ⩽ 1000,$ $1 ⩽ \sum s_i ⩽ 10^6$。
