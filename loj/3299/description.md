...
$1\ t\ x\ y$ 表示一条报名信息，$t = 0$ 时报名战士是冰系，$t = 1$ 时报名战士时火系，$x$ 表示战士的自身温度，$y$ 表示战士的能量。
...
3. 温度为 $100\sim103$ 都能消耗最多的能量 $200$：战士 $1$ 对阵战士 $2$ 消耗能量 $200$，最佳温度为 $103$。
...
6. 温度 $100\sim102$ 能消耗最多的能量 $200$：战士 $2$ 对阵战士 $3$ 消耗能量 $200$，最佳温度为 $102$。
7. 温度 $100\sim104$ 能消耗最多的能量 $700$：首先，战士 $2$ 对阵战士 $7$ 消耗能量 $200$；然后，战士 $4$ 对阵战士 $7$ 消耗能量 $500$，最佳温度为 $104$。
...
$10\%$ 的数据：$Q\le 100, x\le 10^3$。  
另有 $20\%$ 的数据：$Q \le 10^4, x\le 5000$，不存在撤回信息，且输入的 $x$ 按顺序不降。  
$60\%$ 的数据（包含上述 $20\%$，下同）：$Q\le 2\times 10^5, x\le 2\times 10^5$。  
$90\%$ 的数据：$Q\le 2\times 10^6, x\le 2\times 10^6$。  
$100\%$ 的数据：$Q\le 2\times 10^6, x\le 2\times 10^9$，所有 $y$ 之和不超过 $2\times 10^9$，保证不存在 $t,x,y$ 完全相同的两个战士。
